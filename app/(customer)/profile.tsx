...
export default function ProfileScreen() {
  ...
  const backParams = useLocalSearchParams<{ updatedName?: string; updatedPhone?: string }>();
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (response.data) {
          ...
          throw new Error("Invalid response profile format payload from server");
        }
      } catch (err: any) {
    ...
    <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
      ...
      {loading ? (
        ...
        <View className="flex-1 items-center justify-center px-6">
          ...
          <Text className="text-gray-800 font-bold text-lg mt-2">Authentication Issue</Text>
          <Text className="text-gray-500 text-sm text-center mt-1 mb-6">{error}</Text>
          <TouchableOpacity 
        ...
        >
          ...
          <UserInfo
            ...
            imageUrl="https://i.pravatar.cc/150?img=11" // Clean default fallback avatar image link
          />
          ...
          <View className="bg-white rounded-3xl mx-4 py-2 mb-4 shadow-sm border border-gray-100">
            ...
            <MenuItem iconName="chatbubble-ellipses-outline" title="User Reviews" onPress={() => {}} iconColor="#0DCC7B" />
